...
# Search history/Popular cities
- [x] Добавить раздел история поиска дилеров по городам
- [x] Добавить в блок поиска "популярные города(запросы)", в виде строки с топ 10 популярных запросов


- [x] **`ThrottleMiddleware`** — in-memory `_store`, не работает при 2+ Gunicorn workers или рестарте контейнера. Заменить на Redis перед prod.
